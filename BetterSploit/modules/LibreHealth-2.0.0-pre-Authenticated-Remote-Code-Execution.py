#/usr/bin/python3
# (+) Usage:       python LibreHealth-v2.0.0-Authenticated-Remote-Code-Execution.py <WEBAPP_URL> <USERNAME> <PASSWORD>
...
def sig():
    ...
    SIG += F[4]+"_______"+F[2]+"Re written by: Fancy > github.com/boku7"+F[4]+"_____\n"+S[0]
    return SIG
...
if __name__ == "__main__":
    ...
    print(sig())
#1 | INIT
    USERNAME = input(f'{Colors.red}(LibreHealth <2.0 Pre Authenticated Remote Command Execution){Colors.end} [Enter Username]:~#')
    PASSWORD = input(f'{Colors.red}(LibreHealth <2.0 Pre Authenticated Remote Command Execution){Colors.end} [Enter Password]:~#')
    SERVER_URL = input(f'{Colors.red}(LibreHealth <2.0 Pre Authenticated Remote Command Execution){Colors.end} [Enter Target URL]:~#')
